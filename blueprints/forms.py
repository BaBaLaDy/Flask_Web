...
class ID_LoginForm(wtforms.Form):
    id = wtforms.StringField(validators=[length(min=3, max=20)])
    password = wtforms.StringField(validators=[length(min=6, max=30)])


class RegisterForm(wtforms.Form):
    username = wtforms.StringField(validators=[length(min=1, max=20)])
    student_id = wtforms.StringField(validators=[length(min=3, max=30)])
    email = wtforms.StringField(validators=[email()])
    ...
    def validate_student_id(self, field):
        ...
        user_model = UserModel.query.filter_by(id=student_id).first()
        if user_model:
            print("学号已经存在")
            raise wtforms.ValidationError("学号已经存在")

class ID_RegisterForm(wtforms.Form):
    username = wtforms.StringField(validators=[length(min=1, max=20)])
    student_id = wtforms.StringField(validators=[length(min=3, max=30)])
